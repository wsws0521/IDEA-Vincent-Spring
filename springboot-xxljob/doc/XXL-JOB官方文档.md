...
#### 步骤一：执行器项目中，开发Job类：
    ...
    1、开发一个继承自"cn.vincent.xxljob.core.handler.IJobHandler"的JobHandler类，实现其中任务方法。
    2、手动通过如下方式注入到执行器容器。
    ```
...
```
// 可参考Sample示例执行器中的 "cn.vincent.xxljob.executor.service.jobhandler.SampleXxlJob" ，如下：
...
### 5.16 任务失败告警
默认提供邮件失败告警，可扩展短信、钉钉等方式。如果需要新增一种告警方式，只需要新增一个实现 "cn.vincent.xxljob.admin.core.alarm.JobAlarm" 接口的告警实现即可。可以参考默认提供邮箱告警实现 "EmailJobAlarm"。
...
API服务位置：cn.vincent.xxljob.core.biz.AdminBiz （ cn.vincent.xxljob.admin.controller.JobApiController ）
API服务请求参考代码：cn.vincent.xxljob.adminbiz.AdminBizTest
...
API服务位置：cn.vincent.xxljob.core.biz.ExecutorBiz
API服务请求参考代码：cn.vincent.xxljob.executorbiz.ExecutorBizTest
...
请求数据格式如下，放置在 RequestBody 中，JSON格式：
    {
        ...
        "executorBlockStrategy":"COVER_EARLY",      // 任务阻塞策略，可选值参考 cn.vincent.xxljob.core.enums.ExecutorBlockStrategyEnum
        "executorTimeout":0,                        // 任务超时时间，单位秒，大于零时生效
        "logId":1,                                  // 本次调度日志ID
        "logDateTime":1586629003729,                // 本次调度日志时间
        "glueType":"BEAN",                          // 任务模式，可选值参考 cn.vincent.xxljob.core.glue.GlueTypeEnum
...
- 8、任务告警组件模块化：如果需要新增一种告警方式，只需要新增一个实现 "cn.vincent.xxljob.admin.core.alarm.JobAlarm" 接口的告警实现即可，更加灵活、方便定制；
